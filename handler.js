...
esteh: 0,
    ...
    return new Promise(async(resolve, reject) => {
        ...
        resolve(ab)
          ...
          global.kontak2 = [
        ['6283830805068', 'LUKMAN', 'CPX', 'Bot', true],
        [`${nomorown}`, `${nameown}`, 'Lukman', 'Lukman', true]
        ]
        ...
        if (!m.fromMe && opts['self'])
            return
        if (opts['hanya pc'] && m.chat.endsWith('g.us'))
            return
        if (opts['hanya grub'] && !m.chat.endsWith('g.us'))
            return
        if (opts['hanya sw'] && m.chat !== 'status@broadcast')
            return
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.sendButton(m.chat, `[❗] *LIMIT LU HABIS ANJIR,MAKANYA BELIK JAN PELIT KLIK DIBAWAH INI*`, author, null, [['Buy Limit', '/buy limit'], ['Menu', '/menu']] , m)
                    continue // Limit habis
                }
                if (plugin.level > _user.level) {
                    this.sendButton(m.chat, `[💬] Level lu masik dikit📉bro naikin dulu *${plugin.level}* untuk menggunakan perintah in📈i. Level kamu *${_user.level}🧧🏅*\n*${plugin.level}* level is required to use this command. Your level is *${_user.level}🏅*`, author, null,[['Ok', 'ok']] , m)
                    ...
                    if (e) {
                        ...
                        if (e.name)
                            for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                                let data = (await conn.onWhatsApp(jid))[0] || {}
                                if (data.exists)
                                    m.reply(`*BRO ADA YANG ERROR BRO*\n\n_KELACAK_\n\nERORNYA DI PLUGIN:* ${m.plugin}\n* PENGIRIM:* ${m.sender}\n*🗳️ID:* ${m.chat}\n*COMMAND ERRORNYA:* ${usedPrefix}${command} ${args.join(' ')}\n⚠️ *Logs Eror:*\n\n\`\`\`${text}\`\`\``.trim(), data.jid)
                            }
                        m.reply(text)
                    }
                } finally {
 ...
             if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'WELCOME TO CPX Botz🎉🎉, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'unknow') :
                            (chat.sBye || this.bye || conn.bye || 'SELAMAT PERGI🥲, @user!')).replace('@user', `${this.getName(user)}`)
...
Fitur ini di *disable* !`
    ...
    if (msg) return conn.sendButton(m.chat, danied, msg, `${imgr + 'Accses Danied'}`, [['MENU', '.menu'],['OWNER', '.menu']],m)
     ...
     let msgg = {
    	unreg: `${nmsr}\nBRO LOGIN DULU BROO BIAR BISA MAKE BOTNYA *KLIK TOMBOL LOGIN DIBAWAH INI*\n\n*SELAMAT MENGGUNAKAN* ?`
}[type]
