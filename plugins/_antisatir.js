...
export async function before(m, { conn, args, usedPrefix, command, isAdmin, isBotAdmin }) {
    ...
    if (chat.antiSatir && isAntiSatir) {
        await conn.sendButton(m.chat, `*KATA SATIR TERDEREKSI!*${isBotAdmin ? '' : '\n\n_Bot bukan atmin_'}`, author, ['off antisatir', '/disable antisatir'], m)
        if (isBotAdmin && bot.restrict) {
            // await conn.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
    global.db.data.users[m.sender].warn += 1
    
    await conn.sendButton(m.chat, bottime, `Kamu mendapatkan Warn 1 Karena menggunakan Kata yg berbau Satir  Ketik *.my* untuk cek Warning mu`, `${imgr + 'Anti Satir'}`, [
        ['MY', `${usedPrefix}my`]
    ], m)
